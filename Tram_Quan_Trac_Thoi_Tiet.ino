...
void rainInit() {
  ...
  Serial.print("V_AmountOfWater : ");
  Serial.println(V_AmountOfWater);
  Serial.print("S_Funnel : ");
  Serial.println(S_Funnel);
  Serial.print("onePulseValue : ");
  Serial.println(onePulseValue);
...
void windInit() {
  ...
  Serial.print("D_Anemometer : ");
  Serial.println(D_Anemometer);
  Serial.print("C_Anemometer : ");
  Serial.println(C_Anemometer);
    ...
    switch (screenOLED) {
      ...
      case SCREEN1:  // Hiển thị nhiệt độ , độ ẩm
        for (int j = 0; j < 2 && enableShow == ENABLE; j++) {
          ...
          oled.drawCircle(52, 12, 3, SH110X_WHITE);
          oled.print(" C");
      ...
      case SCREEN2:  // Hiển thị bụi
        ...
        if (enableShow == ENABLE)
          screenOLED = SCREEN3;  // chuyển tiếp sang màn hình mưa gió
        break;
...
void connectAPMode() {
  ...
  // Get data từ client
  server.on(
    "/post_data", HTTP_POST, [](AsyncWebServerRequest *request) {
...
void readEEPROM() {
  ...
  for (int i = 64; i < 96; ++i)  // Reading Token
    Etoken += char(EEPROM.read(i));

  if (Essid.length() == 0) Essid = "BLK";

  EtempThreshold1 = EEPROM.read(200);  // ngưỡng nhiệt độ 1
  EtempThreshold2 = EEPROM.read(201);  // ngưỡng nhiệt độ 2

  EhumiThreshold1 = EEPROM.read(202);  // ngưỡng độ ẩm 1
  EhumiThreshold2 = EEPROM.read(203);  // ngưỡng độ ẩm 2

  ErainThreshold1 = EEPROM.read(204) * 100 + EEPROM.read(205);  // ngưỡng mưa 1
  ErainThreshold2 = EEPROM.read(206) * 100 + EEPROM.read(207);  // ngưỡng mưa 2

  EwindThreshold1 = EEPROM.read(211) * 100 + EEPROM.read(212);  // ngưỡng gió 1
  EwindThreshold2 = EEPROM.read(213) * 100 + EEPROM.read(214);  // ngưỡng gió 2

  EdustThreshold1 = EEPROM.read(230) * 100 + EEPROM.read(231);  // ngưỡng bụi 1
  EdustThreshold2 = EEPROM.read(232) * 100 + EEPROM.read(233);  // ngưỡng bụi 2
...
void writeEEPROM() {
  ...
  for (int i = 0; i < Essid.length(); ++i)
    EEPROM.write(i, Essid[i]);  // lưu SSID
  for (int i = 0; i < Epass.length(); ++i)
    EEPROM.write(32 + i, Epass[i]);  // lưu Password
  for (int i = 0; i < Etoken.length(); ++i)
    EEPROM.write(64 + i, Etoken[i]);  // lưu Token
  ...
  // --- Mưa ---
  EEPROM.write(204, ErainThreshold1 / 100);  // lưu hàng trăm + nghìn ngưỡng mưa 1
  EEPROM.write(205, ErainThreshold1 % 100);  // lưu chục + đơn vị ngưỡng mưa 1
  EEPROM.write(206, ErainThreshold2 / 100);  // lưu hàng trăm + nghìn ngưỡng mưa 2
  EEPROM.write(207, ErainThreshold2 % 100);  // lưu chục + đơn vị ngưỡng mưa 2

  // --- Gió ---
  EEPROM.write(211, EwindThreshold1 / 100);  // lưu hàng trăm + nghìn ngưỡng gió 1
  EEPROM.write(212, EwindThreshold1 % 100);  // lưu chục + đơn vị ngưỡng gió 1
  EEPROM.write(213, EwindThreshold2 / 100);  // lưu hàng trăm + nghìn ngưỡng gió 2
  EEPROM.write(214, EwindThreshold2 % 100);  // lưu chục + đơn vị ngưỡng gió 2

  // --- Bụi ---
  EEPROM.write(230, EdustThreshold1 / 100);  // lưu hàng trăm + nghìn ngưỡng bụi 1
  EEPROM.write(231, EdustThreshold1 % 100);  // lưu chục + đơn vị ngưỡng bụi 1
  EEPROM.write(232, EdustThreshold2 / 100);  // lưu hàng trăm + nghìn ngưỡng bụi 2
  EEPROM.write(233, EdustThreshold2 % 100);  // lưu chục + đơn vị ngưỡng bụi 2

  // --- Thời gian lấy mẫu ---
  EEPROM.write(215, ErainTimeSample / 100);  // lưu hàng trăm + nghìn thời gian mẫu mưa
  EEPROM.write(216, ErainTimeSample % 100);  // lưu chục + đơn vị thời gian mẫu mưa
  EEPROM.write(217, EwindTimeSample / 100);  // lưu hàng trăm + nghìn thời gian mẫu gió
  EEPROM.write(218, EwindTimeSample % 100);  // lưu chục + đơn vị thời gian mẫu gió

  // --- Phễu mưa ---
  EEPROM.write(219, (ErainSFunnel / 100) / 100);  // lưu trăm nghìn + chục nghìn diện tích phễu
  EEPROM.write(220, (ErainSFunnel / 100) % 100);  // lưu nghìn + trăm diện tích phễu
  EEPROM.write(221, (ErainSFunnel / 100) % 100);  // lưu chục + đơn vị diện tích phễu

  // --- Lượng nước mưa ---
  EEPROM.write(222, ErainAmountOfWater);  // lưu lượng nước cho 1 lần lật phễu

  // --- Đường kính quạt ---
  EEPROM.write(223, EwindDAnemometer / 100);  // lưu hàng trăm + nghìn đường kính quạt gió
  EEPROM.write(224, EwindDAnemometer % 100);  // lưu chục + đơn vị đường kính quạt gió
 ...
 * @param rain Lượng mưa hiện tại   mm/h
 * @param wind Vận tốc gió hiện tại m/s
 * @param dust Nồng độ bụi PM2.5    ug/m3
 */
void check_data_and_send_to_blynk(int temp, int humi, int rain, int wind, int dust) {
